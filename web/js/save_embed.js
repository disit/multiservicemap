  ...
  //address
   ...
   function save_handler(typeService, id, nameService, embed, textSearch) {
    ...
    if (save_operation == "write") {
      ...
      dialog+="<div id='save_json_div' style=\"display:none;\">";
      ...
      if(format=='json'){
        if(accessTokenMessage==false){
          ...
          dialog+="<br><br>";
          dialog+="<div>";
          dialog+="<div style=\"FLOAT: LEFT;\">";
          dialog+="<input type='button' id='copy_query_json' value='Copy' onclick='copy_query(\"query_link_json\");'>";
          dialog+="</div>";
          dialog+="<div style=\"left: 10px;position: relative;padding-right: 3px;\">";
          dialog+="<span style=\"color: red;font-weight:bold;font-size: 10px;\">Note:</span>";
          dialog+="<span style='font-size: 10px;'>You can add \'&amp;accessToken=\', followed by the access token, to update the results with private/delegated devices </span>";
          dialog+="</div>";
          dialog+="</div>";
          dialog+="</fieldset>";
          dialog+="</div>";
